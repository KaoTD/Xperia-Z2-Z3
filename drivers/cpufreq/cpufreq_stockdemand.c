...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_STOCKDEMAND
static
#endif
struct cpufreq_governor cpufreq_gov_stockdemand = {
       .name                   = "stockdemand",
...
#if 0 /* no longer used in govs, it's in cpufreq.c now */
...
static int stockdemand_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void stockdemand_powersave_bias_init_cpu(int cpu)
{
...
static void stockdemand_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		stockdemand_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_stockdemand Governor Tunables */
...
skip_this_cpu:
		...
		stockdemand_powersave_bias_init();
	} else {
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of stockdemand, waiting for disk IO is an
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	policy = this_dbs_info->cur_policy;
	if (!policy) {
		/* CPU not using stockdemand governor */
...
static int dbs_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (policy->cur < src_freq) {
			/* cancel the next stockdemand sample */
			...
			/* reschedule the next stockdemand sample */
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		stockdemand_powersave_bias_init_cpu(cpu);
		/*
		...
		if (!stockdemand_powersave_bias_setspeed(
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dbs_wq = alloc_workqueue("stockdemand_dbs_wq", WQ_HIGHPRI, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create stockdemand_dbs_wq workqueue\n");
		return -EFAULT;
	}
	...
	return cpufreq_register_governor(&cpufreq_gov_stockdemand);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_stockdemand);
	for_each_possible_cpu(i) {
		struct cpu_dbs_info_s *this_dbs_info =
			&per_cpu(od_cpu_dbs_info, i);
...
MODULE_DESCRIPTION("'cpufreq_stockdemand' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_STOCKDEMAND
